...
console.log();
console.log('* * * Array Metthods -- START * * *')
...
ticketPriceTotal.push(runners.reduce((donationTotal , runner) => {
  return donationTotal += runner.donation;
  // let pushMe = donationTotal += runner.donation;
  // ticketPriceTotal.push(pushMe);
}, 0));
...
// Problem 1
// IT found a phishing attempt coming from various yahoo emails. Get a list of all users with that email so we can take action. 
let yaleEmails = [];
runners.filter(runner => {
  if (runner.email.includes('yale')) {
    yaleEmails.push(runner.email);
  };
});
console.log('yaleEmails()');
console.log(yaleEmails);

// Problem 2
// Let's get the average donations.
let avgDonation = [];
avgDonation.push(runners.reduce((donationTotal , runner) => {
  return donationTotal += runner.donation;
}, 0));
console.log('aveDonation()');
console.log(avgDonation / runners.length);

// Problem 3
// Time to cook the books! Let's start with a clean slate. Return the list with the donations deleted from record. 
let cookedBooks = [];
cookedBooks.push(runners.map(runner => {
  delete runner.donation;
  return runner;
}));
console.log('cookedBooks()');
console.log(cookedBooks);

console.log('* * * Array Metthods -- END * * *')
console.log();
